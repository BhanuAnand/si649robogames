...
while(True):
    ...
    for times in uniqueExpireTime:
        # get the robot data  
        robots = game.getRobotInfo()

    #####----------Social NetWork First Part End-------------------####
    ...
    for k in np.arange(0,101):
        ...
        for times in uniqueExpireTimeGame:
            if (times > k+1):
                ...
                for idx in node_idx:
                    ...
                    #Parent
                    parent_id.append(list(nx.edge_dfs(genealogy, idx, orientation='reverse'))[0][0])
                    ...
                    if (len(predictionHints) > 0):
                        ...
                        #print(idx)
                        #print(predictionHints[predictionHints['id'] == idx])
                        ...
                        VisSub = (c1 + c2 + c3 + c4 + c5).properties(width = 200, height = 200, title = 'Robot ' + str(idx) + ', Expiration time ' + str(expireTime))
                    ...
                    ##################
                    ## TO DO:
                    ## Write the code to create the bar plot for each idx contain the information of productivity of its child, parents
                    ...
                    ##################
                    # if robotInfo is not empty, let's get going
                    if (len(robotInfo) > 0):
        ...
        ###----------Social NetWork Second Part End-------####
